...
const MAX = 255;
let selectedPoint = null;  // keeps last clicked point
...
// Enable/disable based on editor content
...
useCodeBtn.onclick = () => { codeEditor.value = sampleCode; codePopup.style.display='none'; };
...
function updateRedDot(idx) {
  if(idx>=0 && idx<dataPoints.length){
    ...
    drawRedDot(x, y, digital, voltage, t);
  }
}
...
checkCodeBtn.onclick = () => {
  if(normalizeCode(codeEditor.value) === normalizeCode(sampleCode)){
    ...
    canStart = true;
    startSimBtn.disabled = false;   // ✅ enables Start Simulation
  } else {
    alert("Your code is incorrect! Please check again or view assembly code.");
    canStart = false;
    startSimBtn.disabled = true;    // ❌ disables Start Simulation
  }
}

// ---- Simulation ----
function getAnalog(d){ return (d/255)*5; }

function drawGraph(){
  dacGraph.innerHTML = '';
  // polyline
  const points = dataPoints.map((v, idx) => {
    const t = idx * (INTERVAL_MS / 1000); // time in seconds
    const x = 40 + (t / DISPLAY_SECONDS) * VIEW_WIDTH; // scale across 20s
  ...
  for(let v=0; v<=DISPLAY_SECONDS; v++){
    ...
    const line = document.createElementNS('http://www.w3.org/2000/svg','line');
    line.setAttribute('x1',x); line.setAttribute('y1',VIEW_HEIGHT-20);
    line.setAttribute('x2',x); line.setAttribute('y2',VIEW_HEIGHT-15);
    line.setAttribute('stroke','black'); dacGraph.appendChild(line);

    const text = document.createElementNS('http://www.w3.org/2000/svg','text');
    text.setAttribute('x',x-5); text.setAttribute('y',VIEW_HEIGHT-2);
    text.setAttribute('font-size','10'); text.textContent = v+'s';
  ...
  for(let v=0; v<=5; v++){
    const y = VIEW_HEIGHT-20-(v/5)*(VIEW_HEIGHT-40);
    const line = document.createElementNS('http://www.w3.org/2000/svg','line');
    line.setAttribute('x1',35); line.setAttribute('y1',y); line.setAttribute('x2',40); line.setAttribute('y2',y);
    line.setAttribute('stroke','black'); dacGraph.appendChild(line);

    const text = document.createElementNS('http://www.w3.org/2000/svg','text');
    text.setAttribute('x',20); text.setAttribute('y',y+3); text.setAttribute('font-size','10');
    text.textContent = v+'V'; dacGraph.appendChild(text);
  }

  // labels
  const tLabel = document.createElementNS('http://www.w3.org/2000/svg','text');
  tLabel.setAttribute('x',VIEW_WIDTH/2); tLabel.setAttribute('y',VIEW_HEIGHT+35);
  tLabel.setAttribute('font-size','14'); tLabel.setAttribute('text-anchor','middle'); tLabel.setAttribute('fill','black');
  tLabel.textContent = 'Time (s)'; dacGraph.appendChild(tLabel);

  const vLabel = document.createElementNS('http://www.w3.org/2000/svg','text');
  vLabel.setAttribute('x',-VIEW_HEIGHT/2); vLabel.setAttribute('y',15);
  vLabel.setAttribute('transform','rotate(-90)'); vLabel.setAttribute('font-size','14');
  vLabel.setAttribute('text-anchor','middle'); vLabel.setAttribute('fill','black');
  vLabel.textContent='Analog Voltage (V)'; dacGraph.appendChild(vLabel);
}
...
dacGraph.onclick = function(e){
  ...
  const existingDot = dacGraph.querySelector('.red-dot');
  ...
  if(idx>=0 && idx<dataPoints.length){
    selectedPoint = idx;   // remember this point
    updateRedDot(idx);     // draw the dot
    const voltage = dataPoints[idx];
    const t = idx * (INTERVAL_MS / 1000);
    const x = 40 + (t / DISPLAY_SECONDS) * VIEW_WIDTH;
    const y = VIEW_HEIGHT - 20 - (voltage / 5) * (VIEW_HEIGHT - 40);
    ...
    const digital = Math.round((voltage/5)*255);
    dataInfo.textContent = `Time: ${t.toFixed(2)}s | P1: ${digital} | Analog Voltage: ${voltage.toFixed(2)}V`;
...
startSimBtn.onclick = () => {
  ...
  let step = 0;
  const totalSteps = DISPLAY_SECONDS * (1000 / INTERVAL_MS); // total points
  const rampSteps = 5 * (1000 / INTERVAL_MS); // steps per 5s ramp
  ...
  intervalId = setInterval(() => {
    ...
    // Current step within the 5s ramp
    let rampStep =(step % rampSteps);

    // Compute DAC value (0 → 255)
    ...
    drawGraph();
    if (dragPoint !== null) {
        updateRedDot(dragPoint);
    } else if (selectedPoint !== null) {
        updateRedDot(selectedPoint);
    }
...
resetSimBtn.onclick = ()=>{
  ...
  canStart=false;
  downloadCodeBtn.disabled = true;
  startSimBtn.disabled = true;   // 🔒 lock again after reset
  drawGraph();
}
...
downloadCodeBtn.onclick = ()=>{
  ...
  link.download = 'dac_code.asm'; document.body.appendChild(link); link.click();
  document.body.removeChild(link); URL.revokeObjectURL(link.href);
}

// ---- Download graph ----
downloadGraphBtn.onclick = ()=>{
  import('https://html2canvas.hertzen.com/dist/html2canvas.min.js').then(()=>html2canvas(document.querySelector('.graph-container'))).then(canvas=>{
    const link = document.createElement('a'); link.download='dac_ramp_waveform.png';
...
dacGraph.addEventListener('mousedown', function(e){
  const pt = dacGraph.createSVGPoint();
  pt.x = e.clientX; pt.y = e.clientY;
  const cursor = pt.matrixTransform(dacGraph.getScreenCTM().inverse());
  const time = ((cursor.x - 40) / VIEW_WIDTH) * DISPLAY_SECONDS;
...
dacGraph.addEventListener('mousemove', function(e){
  if(!dragging || dragPoint === null) return;
  const pt = dacGraph.createSVGPoint();
  pt.x = e.clientX; pt.y = e.clientY;
  const cursor = pt.matrixTransform(dacGraph.getScreenCTM().inverse());
  let y = Math.min(Math.max(cursor.y, 20), VIEW_HEIGHT-20);
  let v = 5 * (VIEW_HEIGHT-20 - y) / (VIEW_HEIGHT-40);
  dataPoints[dragPoint] = v;
  updateRedDot(dragPoint);
  drawGraph();
});

dacGraph.addEventListener('mouseup', function(e){
  dragging = false;
  dragPoint = null;
});
dacGraph.addEventListener('mouseleave', function(e){
  dragging = false;
  dragPoint = null;
});

function drawRedDot(x, y, digital, voltage, time){
  // remove existing dot
  const existingDot = dacGraph.querySelector('.red-dot');
  if(existingDot) existingDot.remove();
  ...
  dataInfo.textContent = `Time: ${time.toFixed(2)}s | P1: ${digital} | Analog Voltage: ${voltage.toFixed(2)}V`;
}
